...
export default function RegisterScreen({ onBack, onRegisterSuccess }: { onBack: () => void, onRegisterSuccess: () => void }) {
  ...
  const handleRegister = async () => {
    if (!name || !email || !password) return Alert.alert("Hata", "Lütfen tüm alanları doldur!");

    setLoading(true);
    try {
      const result = await authService.register({ name, email, password });
      if (result.status === "success") {
        Alert.alert("Başarılı", "Hesabın oluşturuldu! Şimdi giriş yapabilirsin.", [{ text: "Tamam", onPress: onRegisterSuccess }]);
      } else {
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
          <SafeAreaView style={styles.container}>
            ...
            <View style={styles.form}>
              <TextInput style={styles.input} placeholder="Ad Soyad" placeholderTextColor={COLORS.textSecondary} value={name} onChangeText={setName} />
              ...
              <TextInput style={styles.input} placeholder="Şifre" placeholderTextColor={COLORS.textSecondary} value={password} onChangeText={setPassword} secureTextEntry />
              <TouchableOpacity style={[styles.regBtn, loading && { opacity: 0.7 }]} onPress={handleRegister} disabled={loading}>
                <Text style={styles.regBtnText}>{loading ? "Kaydediliyor..." : "Kaydı Tamamla"}</Text>
              </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  regBtn: { backgroundColor: COLORS.primary, padding: 18, borderRadius: 15, alignItems: 'center', marginTop: 10 },
